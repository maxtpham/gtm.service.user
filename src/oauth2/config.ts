...
export function normalizeOAuth2(config: IOAuth2Config): IOAuth2Config {
    ...
    if (config.auth.google) {
        ...
        if (!config.auth.google.options) config.auth.google.options = {} as IOAuth2ProviderOptionConfig;
    ...
    if (config.auth.facebook) {
        ...
        if (!config.auth.facebook.options) config.auth.facebook.options = {} as IOAuth2ProviderOptionConfig;
        if (typeof(process.env.AUTH_FACEBOOK_OPTIONS_CLIENTID) === 'string') config.auth.facebook.options.clientID = process.env.AUTH_FACEBOOK_OPTIONS_CLIENTID;
